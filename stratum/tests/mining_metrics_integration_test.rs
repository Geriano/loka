...
mod tests {
    ...
    async fn test_mining_metrics_operations() {
        ...
        for i in 0..5 {
            ...
            metrics
                .job_distribution_latency_sum_ms_bits
                .store((prev_sum + latency_ms).to_bits(), Ordering::Relaxed);
            metrics
                .job_distribution_latency_count
                .fetch_add(1, Ordering::Relaxed);

            // Update min/max
            let prev_min = f64::from_bits(
                metrics
                    .job_distribution_latency_min_ms_bits
                    .load(Ordering::Relaxed),
            );
            let prev_max = f64::from_bits(
                metrics
        ...
        for i in 0..20 {
            ...
            if valid {
                ...
                metrics
                    .share_acceptance_rate_sum_bits
                    .store((prev_sum + 1.0).to_bits(), Ordering::Relaxed);
            } else {
        ...
        let acceptance_rate = f64::from_bits(
            metrics
                .share_acceptance_rate_sum_bits
                .load(Ordering::Relaxed),
        ) / metrics.share_acceptance_rate_count.load(Ordering::Relaxed)
            as f64;
        assert!(
    ...
    fn test_direct_atomic_metrics() {
        ...
        metrics
            .current_difficulty_bits
            .store((1500.0_f64).to_bits(), Ordering::Relaxed);
        assert_eq!(
            metrics
                .difficulty_adjustments_counter
                .load(Ordering::Relaxed),
        ...
        metrics
            .job_distribution_latency_sum_ms_bits
            .store((45.0_f64).to_bits(), Ordering::Relaxed);
        metrics
            .job_distribution_latency_count
            .store(3, Ordering::Relaxed);
        let avg_latency = f64::from_bits(
