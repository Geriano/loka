...
impl MockWorker {
    pub fn new(
        ...
        let hashrate_sim = HashrateSimulator::new(config.hashrate_mhs, config.difficulty_variance);
    ...
    async fn run_connection(&self) -> Result<()> {
        ...
        info!(
            "Worker {} connected to {}",
            self.worker_name, self.config.pool_address
        );
        ...
        // Subscribe to mining
        self.send_message(&mut writer, &StratumMessages::mining_subscribe())
            .await?;
    ...
    async fn handle_message(
        &self,
        message: &str,
        message_tx: &Arc<tokio::sync::mpsc::Sender<serde_json::Value>>,
    ) -> Result<()> {
                ...
                "mining.notify" => {
                    if let Some(params) = msg["params"].as_array() {
                        ...
                        if let Some(job) = MiningJob::from_notify_params(params, difficulty) {
                            debug!(
                                "Worker {} received new job: {}",
                                self.worker_name, job.job_id
                            );
                ...
                "mining.set_difficulty" => {
                    if let Some(params) = msg["params"].as_array() {
                        if let Some(difficulty) = params.get(0).and_then(|v| v.as_f64()) {
                            debug!(
                                "Worker {} difficulty set to: {}",
                                self.worker_name, difficulty
                            );
            ...
            if let Some(result) = msg.get("result") {
                if let Some(subscribe_result) = result.as_array() {
                    ...
                    if subscribe_result.len() >= 3 {
                        ...
                        // Now authorize
                        let auth_msg = StratumMessages::mining_authorize(
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                if config.log_mining {
                    debug!(
                        "Worker {} submitted share for job {}",
                        worker_name, final_job_id
                    );
                }
            }
        })
