...
mod tests {
    ...
    async fn test_bitcoin_standard_hashrate_calculation() {
        ...
        println!("✅ Basic hashrate test: {:.2} H/s ({:.2} MH/s)", hashrate, hashrate / 1_000_000.0);
    }
    ...
    async fn test_hashrate_with_different_difficulties() {
        ...
        for (shares, time_secs, difficulty, description) in test_cases {
            let hashrate = HashUtils::calculate_hashrate_bitcoin_standard(shares, time_secs, difficulty);
            let expected = (shares as f64 * difficulty * 4_294_967_296.0) / time_secs as f64;
            
            assert!(
                (hashrate - expected).abs() < 1.0,
                "{} failed: expected {:.2}, got {:.2}",
                description, expected, hashrate
            );
            
            println!("✅ {}: {:.2} H/s ({:.2} MH/s)", 
                     description, hashrate, hashrate / 1_000_000.0);
        }
    }
    ...
    async fn test_edge_cases() {
        // Test edge cases
        assert_eq!(HashUtils::calculate_hashrate_bitcoin_standard(0, 60, 1.0), 0.0, "Zero shares should return 0");
        assert_eq!(HashUtils::calculate_hashrate_bitcoin_standard(10, 0, 1.0), 0.0, "Zero time should return 0");
    ...
    async fn test_atomic_metrics_hashrate_integration() {
        ...
        println!("✅ Atomic metrics hashrate: {:.2} H/s ({:.2} MH/s)", 
                 hashrate, hashrate / 1_000_000.0);
    ...
    async fn test_realistic_mining_scenario() {
        ...
        if hashrate > 0.0 {
            println!("✅ Realistic scenario hashrate: {:.2} H/s ({:.2} MH/s)", 
                     hashrate, hashrate / 1_000_000.0);
            ...
            assert!(
                ...
                hashrate, min_expected, max_expected
            );
        } else {
    ...
    async fn test_multiple_difficulty_adjustments() {
        ...
        for (i, &diff) in difficulties.iter().enumerate() {
            ...
            assert!((current_diff - diff).abs() < 0.001, 
                    "Difficulty should be updated correctly");
            
            println!("✅ Step {}: Difficulty {}, Hashrate {:.2} H/s", 
                     i + 1, current_diff, hashrate);
        }
    }
    ...
    async fn test_share_counting_accuracy() {
        ...
        let initial_shares = metrics.share_submissions_total.load(std::sync::atomic::Ordering::Relaxed);
        ...
        let final_shares = metrics.share_submissions_total.load(std::sync::atomic::Ordering::Relaxed);
        assert_eq!(final_shares - initial_shares, 25, "Should count exactly 25 shares");
        ...
        let total_shares = metrics.share_submissions_total.load(std::sync::atomic::Ordering::Relaxed);
        assert_eq!(total_shares - initial_shares, 40, "Should count all submissions");
        
        println!("✅ Share counting accuracy verified: {} total shares", 
...
pub mod test_helpers {
    ...
    pub async fn simulate_mining_period(
        ...
        for _minute in 0..minutes {
            ...
            if _minute % 5 == 0 { // Calculate hashrate every 5 minutes
                metrics.calculate_and_update_hashrate(300.0);
            }
        }
    }
    ...
    pub fn assert_hashrate_in_bounds(actual: f64, shares: u64, time_secs: u64, difficulty: f64, tolerance_percent: f64) {
        let expected = HashUtils::calculate_hashrate_bitcoin_standard(shares, time_secs, difficulty);
        let tolerance = expected * (tolerance_percent / 100.0);
        
        assert!(
            (actual - expected).abs() <= tolerance,
            "Hashrate {:.2} not within {:.1}% of expected {:.2} (tolerance: {:.2})",
            actual, tolerance_percent, expected, tolerance
        );
    }
}