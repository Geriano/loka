...
impl MockPool {
    ...
    async fn handle_message(
        ...
        if self.config.should_inject_error() {
            let error_response = MockResponses::error_response(
                msg["id"].as_u64(),
                "Random injected error",
                99,
            );
        ...
        let response = match method {
            ...
            "mining.authorize" => {
                self.handle_authorize(session_id, params, id.unwrap_or(0)).await?
            }
            "mining.submit" => {
                self.handle_submit(session_id, params, id.unwrap_or(0)).await?
            }
            "mining.extranonce.subscribe" => {
                MockResponses::extranonce_subscribe_response(id.unwrap_or(0))
            }
            "mining.ping" => {
                MockResponses::ping_response(id.unwrap_or(0))
            }
            "mining.get_version" => {
                MockResponses::get_version_response(id.unwrap_or(0))
            }
            _ => {
                MockResponses::unknown_method_response(id.unwrap_or(0), method)
            }
        };
    ...
    async fn handle_submit(
        ...
        match self.validator.validate_share(&submission, is_stale).await {
            Ok(true) => {
                Ok(MockResponses::submit_response(id, true, None))
            }
            Ok(false) => {
                Ok(MockResponses::submit_response(id, false, Some("Share validation failed".to_string())))
            }
            Err(e) => {
                Ok(MockResponses::submit_response(id, false, Some(e.to_string())))
            }
        }
    }
