...
impl ProtocolHandler {
                ...
                match reader.read_line(&mut buffer).await {
                    ...
                    Ok(_) => {
                        ...
                        if Self::is_direct_http_request(trimmed_message) {
                            ...
                            if let Some(extracted_path) = Self::extract_path_from_message(
                                &raw_message,
                                Some(&manager),
                                Some(addr),
                            ) {
    ...
    /// Extracts path information from HTTP or Stratum messages.
    ///
    /// This function analyzes incoming messages to detect the protocol type
    /// and extract path information for routing purposes.
    ///
    /// # Arguments
    /// * `raw_message` - The raw message content to analyze
    /// * `manager` - Optional manager for metrics recording
    /// * `addr` - Optional socket address for metrics context
    ///
    /// # Returns
    /// * `Some(String)` - Extracted path if found
    /// * `None` - No path information could be extracted
    pub fn extract_path_from_message(
        raw_message: &str,
        ...
        // Try HTTP protocol first
        if let Some(path) = Self::try_extract_http_path(trimmed, manager, addr) {
            return Some(path);
        }

        // Try Stratum protocol
        Self::try_extract_stratum_path(raw_message, manager, addr)
    }

    /// Attempts to extract path from HTTP request messages.
    fn try_extract_http_path(
        message: &str,
        manager: Option<&Manager>,
        addr: Option<SocketAddr>,
    ) -> Option<String> {
        if let Some(path) = Self::parse_http_request_path(message) {
            Self::record_protocol_detection(manager, addr, "HTTP", true);
            Some(path)
        } else {
            None
        }
    ...
    /// Attempts to extract path from Stratum JSON messages.
    fn try_extract_stratum_path(
        raw_message: &str,
        manager: Option<&Manager>,
        addr: Option<SocketAddr>,
    ) -> Option<String> {
        match serde_json::from_str::<Value>(raw_message) {
            Ok(json) => {
                Self::record_protocol_detection(manager, addr, "Stratum", true);
                Self::extract_path_from_json(&json)
            }
            Err(_) => {
                Self::record_protocol_detection(manager, addr, "Unknown", false);
                None
            }
        }
    }

    /// Extracts path information from parsed JSON (Stratum protocol).
    fn extract_path_from_json(json: &Value) -> Option<String> {
        // Check mining.subscribe method specifically
    ...
    /// Extracts path from mining.subscribe method parameters.
    fn extract_path_from_mining_subscribe(json: &Value) -> Option<String> {
        ...
        let params = json.get("params")?.as_array()?;

        // Check first parameter (user agent)
        if let Some(user_agent) = params.get(0)?.as_str() {
            if let Some(path) = Self::parse_url_path_from_string(user_agent) {
                return Some(path);
            }
        }

        // Check all parameters for URL patterns
        for param in params {
    ...
    /// Records protocol detection metrics if manager and address are available.
    fn record_protocol_detection(
        manager: Option<&Manager>,
        addr: Option<SocketAddr>,
        protocol: &str,
        success: bool,
    ) {
        if let (Some(manager), Some(addr)) = (manager, addr) {
            match (protocol, success) {
                ("HTTP", true) => {
                    manager.protocol_http_request_detected(&addr);
                    manager.protocol_detection_success(&addr, "HTTP");
                }
                ("Stratum", true) => {
                    manager.protocol_stratum_request_detected(&addr);
                    manager.protocol_detection_success(&addr, "Stratum");
                }
                (_, false) => {
                    manager.protocol_detection_failure(&addr);
                }
                _ => {}
            }
        }
