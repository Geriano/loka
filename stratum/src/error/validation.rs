...
pub trait MessageValidator: Send + Sync {
    ...
    fn should_validate(&self, _method: Option<&str>) -> bool {
            ...
            Value::String(s) => {
                if s.len() > self.max_string_length {
                    return Err(StratumError::ValidationFailed {
                        ...
                        message: format!(
                            "String length {} exceeds limit of {}",
                            s.len(),
            ...
            Value::Object(obj) => {
                if obj.len() > self.max_object_properties {
                    return Err(StratumError::ValidationFailed {
                        ...
                        message: format!(
                            "Object properties {} exceed limit of {}",
                            obj.len(),
...
impl MessageValidator for JsonStructureValidator {
    async fn validate(&self, message: &Value, _context: &ValidationContext) -> Result<()> {
...
impl RateLimitValidator {
    ...
    fn cleanup_old_entries(&self) {
        ...
        debug!(
            "Cleaned up rate limit entries, {} IPs tracked",
...
mod tests {
    ...
    async fn test_rate_limit_validator() {
        ...
        let context = ValidationContext::new().with_remote_addr(SocketAddr::new(ip, 1234));
    ...
    async fn test_composite_validator() {
        ...
        let context = ValidationContext::new().with_method("mining.authorize".to_string());
